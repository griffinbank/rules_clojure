...
(s/def ::ns-path (s/map-of symbol? path?))
(s/def ::read-deps map?)
...
(s/def ::jar->lib (s/map-of path? ::library))
...
(defn get! [m k]
  (assert (find m k) (print-str "could not find" k "in" m))
  (get m k))
...
(s/fdef jar->label :args (s/cat :a (s/keys :req-un [::deps-repo-tag ::jar->lib]) :p path?) :ret string?)
(defn jar->label
...
(defn ns-deps
  ...
  (try
    (->> ns-decl
         ...
         (map (partial ns->label (select-keys args [:ns->path :jar->lib :workspace-root :deps-repo-tag])))
         (filter identity)
         (distinct)
...
(defn ns-rules
  ...
  (let [overrides (get-in deps-bazel [:extra-deps (src->label {:workspace-root workspace-root} path)])]
    (when overrides
...
(s/fdef make-basis :args (s/cat :a (s/keys :req-un [::read-deps ::aliases ::deps-out-dir])) :ret ::basis)
(defn make-basis
  "combine a set of aliases and return a complete deps map"
  [{:keys [read-deps aliases deps-out-dir]}]
  ...
  (spit (-> (->path deps-out-dir "BUILD.bazel") path->file)
        (str/join "\n\n" (concat
                          [(emit-bazel (list 'package {:default_visibility ["//visibility:public"]}))
                           ...
                           (emit-bazel (list 'sh_binary {:name "gen_build"
                                                         :srcs ["@rules_clojure//scripts:gen_build.sh"]}))]
                          (->> jar->lib
                               (map (fn [[jarpath lib]]
                                      (let [munged (str (library->label lib))
                                            extra-deps (get-in deps-bazel [:extra-deps (jar->label (select-keys args [:deps-repo-tag :jar->lib]) jarpath)])]
                                        (when extra-deps
                                        ...
                                        (emit-bazel (list 'java_import (merge-with into
                                                                                   {:name munged
                                                                                    :jars [(str "//:" (path-relative-to deps-out-dir jarpath))]
                                                                                    :deps (->> (get lib->deps lib)
                                                                                               (mapv (fn [lib]
                                                                                                       (str deps-repo-tag "//:" (library->label lib)))))}
                                                                                   extra-deps)))))))))
        :encoding "UTF-8"))
...
(defn -main [{:keys [deps-out-dir deps-edn-path deps-repo-tag aliases]
