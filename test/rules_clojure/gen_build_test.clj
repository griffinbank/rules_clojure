...
(deftest gen-dir-writes-nothing-for-empty-dirs
  (testing "directory with no clj content: no BUILD.bazel is written (empty stubs are no longer emitted)"
    (let [content (run-gen-dir! {})]
      (is (nil? content)
          "should not write a BUILD.bazel for a directory with no clojure content"))))
...
(def ^:private generated-stub
  "A stub BUILD.bazel matching what gen_srcs writes for a no-content directory."
  (str "\"\"\"\nDo not edit this file manually!\n"
       "It is automatically generated by the `//:gen_srcs` target from `rules_clojure`.\n\"\"\"\n\n"
       "package(default_visibility = [\"//visibility:public\"])\n"))

(defn- run-gen-dir-with-build!
  "Set up a temp dir, write `files` (relative-path → content) under src/example, and
   pre-create src/example/BUILD.bazel with `build-content` (when non-nil). Call gen-dir
   and return {:result <result map> :exists? <does BUILD.bazel exist after> :content <its content or nil>}."
  [files build-content & {:keys [dep-ns->label]
                          :or {dep-ns->label {:clj {} :cljs {}}}}]
  (let [dir (make-temp-dir)
        src-dir (fs/->path (.getAbsolutePath dir) "src" "example")]
    (try
      ...
      (let [build-file (io/file (.toFile src-dir) "BUILD.bazel")]
        (when build-content
          (spit build-file build-content))
        (let [result (gb/gen-dir (minimal-args dir dep-ns->label) src-dir)]
          {:result result
           :exists? (.exists build-file)
           :content (when (.exists build-file) (slurp build-file))}))
      (finally
        (fs/rm-rf (.toPath dir))))))

(deftest gen-dir-deletes-generated-stub
  (testing "a no-content dir with a previously-generated stub BUILD.bazel has it deleted"
    (let [{:keys [result exists?]} (run-gen-dir-with-build! {} generated-stub)]
      (is (not exists?) "generated stub should be deleted")
      (is (= 1 (:deleted result)) "result should report :deleted 1")
      (is (= 0 (:wrote result)) "nothing should be written"))))

(deftest gen-dir-keeps-hand-written-build
  (testing "a no-content dir with a hand-written BUILD.bazel (no generated header) is left untouched"
    (let [hand-written "# hand-written\ncc_library(name = \"resources\")\n"
          {:keys [result exists? content]} (run-gen-dir-with-build! {} hand-written)]
      (is exists? "hand-written BUILD.bazel should not be deleted")
      (is (= hand-written content) "hand-written content should be untouched")
      (is (= 0 (:deleted result)) "result should report :deleted 0"))))

(deftest gen-dir-keeps-stub-when-subdir-has-content
  (testing "a dir whose only clojure content lives in a subdir keeps/rewrites its BUILD.bazel (regression guard)"
    (let [{:keys [result exists?]} (run-gen-dir-with-build!
                                    {"child/core.clj" "(ns example.child.core)"}
                                    generated-stub)]
      (is exists? "BUILD.bazel should not be deleted when a subdir has clojure content")
      (is (nil? (:deleted result)) "delete branch should not run when has-content? is true"))))
