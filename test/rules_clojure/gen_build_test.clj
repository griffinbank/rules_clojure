...
(deftest rollup-rules-matches-shared-parity-fixtures
  (testing "shared fixtures pin the cross-process contract between
            gen_build.clj's rollup-rules and gazelle_server.bb's rollup-rules.
            Both implementations load this same fixture file; drift between
            them surfaces here OR in the bb-side mirror test instead of
            silently producing divergent BUILD output."
    (let [fixture-path (first (test-utils/runfiles-env "ROLLUP_FIXTURES"))
          fixtures (read-string (slurp fixture-path))]
      (doseq [{:keys [name input expected]} fixtures]
        (testing (str "fixture: " name)
          (is (= expected (gb/rollup-rules input))))))))
