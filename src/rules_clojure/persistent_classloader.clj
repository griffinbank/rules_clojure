...
(defn dirty-fast-discard
  "Use a single classloader for all compiles, discarding the classloader when :aot-nses contain protocols. Works, only in single-threaded mode"
  []
  (let [classloader (volatile! nil)
        loaded? (volatile! nil)
        contains-protocols? (volatile! nil)
        reload! (fn [new-classpath]
                  (vreset! classloader (new-classloader- new-classpath))
                  (vreset! loaded? (util/invoker-1 @classloader "rules-clojure.compile" "loaded?"))
                  (vreset! contains-protocols? (util/invoker-1 @classloader "rules-clojure.compile" "contains-protocols-or-deftypes?")))
        init! (fn [classpath]
                (when-not @classloader
                  (reload! classpath))
                (doseq [p classpath]
    ...
    (reify ClassLoaderStrategy
      (with-classloader [this {:keys [classpath aot-nses]} f]
        ...
        (when (some (fn [ns] (@loaded? ns)) aot-nses)
          (reload! classpath))
