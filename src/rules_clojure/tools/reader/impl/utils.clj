...
(ns ^:skip-wiki rules-clojure.tools.reader.impl.utils
...
(compile-when <=clojure-1-7-alpha5
  ...
  (defn tagged-literal?
    ...
    [value]
    (instance? rules_clojure.tools.reader.impl.utils.TaggedLiteral value))

  (defn tagged-literal
    "Construct a data representation of a tagged literal from a
       tag symbol and a form."
    [tag form]
    (rules_clojure.tools.reader.impl.utils.TaggedLiteral. tag form))

  (ns-unmap *ns* '->TaggedLiteral)
  (ns-unmap *ns* 'map->TaggedLiteral)

  (defmethod print-method rules_clojure.tools.reader.impl.utils.TaggedLiteral [o ^java.io.Writer w]
    (.write w "#")
  ...
  (defn reader-conditional?
    ...
    [value]
    (instance? rules_clojure.tools.reader.impl.utils.ReaderConditional value))

  (defn reader-conditional
    "Construct a data representation of a reader conditional.
       If true, splicing? indicates read-cond-splicing."
    [form splicing?]
    (rules_clojure.tools.reader.impl.utils.ReaderConditional. splicing? form))

  (defmethod print-method rules_clojure.tools.reader.impl.utils.ReaderConditional [o ^java.io.Writer w]
    (.write w "#?")
