  ...
  (try
    (if-let [[_ns ns-name & refs :as ns-decl] (get-ns-decl path)]
      (let [test? (test-ns? path)
        ...
        (->>
         (concat
          [(emit-bazel (list 'clojure_namespace (kwargs (-> (merge-with into
                                                                        {:name ns-label
...
(defn gen-resources
  ...
  (spit (-> deps-edn-path dirname (->path path "BUILD.bazel") path->file)
        (str "#autogenerated, do not edit\n"
             (emit-bazel (list 'package (kwargs {:default_visibility ["//visibility:public"]})))
             "\n"
             (emit-bazel (list 'load "@rules_clojure//:rules.bzl" "clojure_namespace", "clojure_library"))
             "\n"
             (emit-bazel (list 'clojure_namespace (kwargs {:name (str (basename path) "_ns")
                                                           :srcs (->> (ls-r path)
                                                                      (filter (fn [p]
