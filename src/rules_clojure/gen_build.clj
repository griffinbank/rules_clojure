...
(defn emit-bazel-kwargs
  "Return a seq of \"k = v\" entry strings, sorted by buildifier attribute priority.
   ...
   Entries whose value is an empty collection are dropped (e.g. `data = []` /
...
(defn rollup-rules
  ...
  Inputs (each is a sequential collection of strings, or nil treated as empty):
    :lib-deps               local target names in this package (e.g. [\"core\" \"util\"]). Use the
                            :name attrs of the emitted clojure_library/java_library rules (callers
                            filter rollup-kinds to those two, so basename / :name agreement holds
                            for the rule kinds we consume).
...
(defn gen-dir
  ...
  (let [paths (->> (fs/ls dir)
        ...
        lib-names (into [] (comp (filter (comp rollup-kinds :type))
                                 (map (comp :name :attrs))
                                 (distinct))
                        rule-specs)
        has-binary? (some #(= :clojure_binary (:type %)) rule-specs)
        has-tests?  (some #(= :clojure_test (:type %)) rule-specs)
        rules (mapv emit-rule rule-specs)
