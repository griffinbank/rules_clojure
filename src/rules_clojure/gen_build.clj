...
(s/fdef jar->label :args (s/cat :a (s/keys :req-un [::jar->lib] :opt-un []) :p fs/path?) :ret string?)
(defn jar->label
  "Given a .jar path, return the bazel label"
  [{:keys [jar->lib] :as args} jarpath]
  (str ":" (->> jarpath (get! jar->lib) library->label)))
...
(defn ns->label
  ...
  [{:keys [src-ns->label dep-ns->label] :as args} ns platform]
  {:pre [(keyword? platform)]}
...
(defn ns-deps
  ...
  [{:keys [src-ns->label dep-ns->label jar->lib] :as args} ns-decl platform]
  (let [ns-name (-> ns-decl second)]
    (try
      (->> ns-decl
           ...
           (map (fn [ns]
                  (ns->label (select-keys args [:src-ns->label :dep-ns->label]) ns platform)))
           (filter identity)
           (distinct)
...
(s/fdef ns-import-deps :args (s/cat :a (s/keys :req-un [::class->jar ::jar->lib]) :n ::ns-decl) )
(defn ns-import-deps
  "Given the ns declaration for a .clj file, return a map of {:srcs [labels], :data [labels]} for all :import statements"
  [{:keys [class->jar jar->lib] :as args} ns-decl]
  (assert class->jar)
    ...
    (->> refs
         ...
         (map (fn [class]
                (when-let [jar (get class->jar class)]
                  {:deps [(jar->label {:jar->lib jar->lib} jar)]})))
         (filter identity)
         (distinct)
...
(defn ns-rules
  ...
  [{:keys [basis deps-bazel deps-edn-dir] :as args} paths]
  ...
  (try
    (let [clj? (some clj-path? paths)
          ...
          deps (apply merge-with into (map (fn [[decl platform]]
                                             (merge-with into
                                                         (ns-deps (select-keys args [:deps-edn-dir :src-ns->label :dep-ns->label :jar->lib]) decl platform)
...
(s/fdef gen-dir :args (s/cat :a (s/keys :req-un [::deps-edn-dir ::basis ::jar->lib]) :f fs/path?))
(defn gen-dir
...
(defn gen-deps-build
  ...
  [{:keys [repository-dir deps-build-dir dep-ns->label jar->lib lib->jar lib->deps deps-bazel] :as args}]
                                            ...
                                            (concat
                                             ...
                                             (->>
                                              ...
                                              (find/sources-in-jar jarfile find/clj)
                                               ...
                                               (map (fn [[ns ns-decls]]
                                                      (let [extra-deps (-> deps-bazel (get-in [:deps (str ":" (internal-dep-ns-aot-label lib ns))]))]
                                                        (emit-bazel (list 'clojure_library (kwargs (->
                                                                                                     (merge-with
                                                                                                       ...
                                                                                                       {:name (internal-dep-ns-aot-label lib ns)
                                                                                                        ...
                                                                                                        :runtime_deps []}
                                                                                                       (apply merge-with into (map #(ns-deps (select-keys args [:dep-ns->label :jar->lib]) % :clj) ns-decls))
                                                                                                       extra-deps)
                                                                                                     (as-> m
                                                                                                       (cond-> m
