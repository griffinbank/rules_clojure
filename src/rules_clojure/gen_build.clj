...
(defn gen-resources
  ...
  (spit (-> deps-edn-path dirname (->path path "BUILD.bazel") path->file)
        (str "#autogenerated, do not edit\n"
             (emit-bazel (list 'package {:default_visibility ["//visibility:public"]}))
             "\n"
             (emit-bazel (list 'load "@rules_clojure//:rules.bzl" "clojure_namespace"))
             "\n"
                                                   ...
                                                   :srcs (->> (ls-r path)
                                                              ...
                                                              (map (fn [f]
                                                                     (let [package-path (path-relative-to workspace-root path)
                                                                       ...
                                                                       [label (str "/" file-path)])))
                                                              (into {}))}))
             (emit-bazel (list 'clojure_library {:name "resources.jar"
                                                 :deps [(basename path)]}))
             "\n")))
...
(defn gen-resources
  ...
  (spit (-> deps-edn-path dirname (->path path "BUILD.bazel") path->file)
        (str "#autogenerated, do not edit\n"
             ...
             (emit-bazel (list 'load "@rules_clojure//:rules.bzl" "clojure_namespace"))
             "\n"
             (emit-bazel (list 'java_library {:name (basename path)
                                              :data (list 'glob ["**/*"])}))
             "\n")))
