...
(defn- sort-kwargs-entries
  "Sort kwargs entries by buildifier priority: name first, then by priority bucket, then alphabetically within bucket."
  [entries]
  (sort-by (fn [[k _]]
             [(get attr-priority k 0) (name k)])
           entries))
...
(defn- emit-vector-multiline
  "Emit vector with each element on its own line at 8-space indent, trailing commas."
  [items]
  (str "[\n"
       (str/join "\n" (map #(str "        " (emit-bazel* %) ",") items))
       "\n    ]"))

(defn emit-bazel-kwargs [kwargs]
  {:pre [(map? kwargs)]
   :post [(string? %)]}
  (let [entries (sort-kwargs-entries (seq (:x kwargs)))]
    (->> entries
         (map (fn [[k v]]
                (let [v-str (if (and (vector? v) (> (count v) 1))
                              (emit-vector-multiline
                                (if (sortable-list-attrs k)
                                  (sort-by label-sort-key v)
                                  v))
                              (emit-bazel* v))]
...
(defmethod emit-bazel* IPersistentList [[fn-name & args]]
  ;; function call
  (let [kw-args (first (filter kwargs? args))
        pos-args (remove kwargs? args)]
    (if (or (nil? kw-args) (<= (count (:x kw-args)) 1))
      ;; inline: load("a", "b") or package(key = val)
      (let [parts (concat
                    (map emit-bazel* pos-args)
                    (when kw-args
                      (map (fn [[k v]]
                             (str (emit-bazel* k) " = " (emit-bazel* v)))
                           (:x kw-args))))]
        (str fn-name "(" (str/join ", " parts) ")"))
      ;; multiline: rule(\n    key = val,\n)
      (str fn-name "(\n" (emit-bazel-kwargs kw-args) "\n)"))))
