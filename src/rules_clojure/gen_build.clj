...
(defn gen-dir
  ...
  (let [paths (->> (fs/ls dir)
    ...
    (let [build-file (-> dir (fs/->path "BUILD.bazel") fs/path->file)
      ...
      (if has-content?
        (let [changed? (not= content (when exists? (slurp build-file :encoding "UTF-8")))]
          (when changed?
            (spit build-file content :encoding "UTF-8"))
          {:files (count paths) :wrote (if changed? 1 0)})
        ;; nothing clojure here or below — don't leave a stub BUILD.bazel behind
        (let [generated? (and exists?
                              (str/includes? (slurp build-file :encoding "UTF-8")
                                             "automatically generated by the `//:gen_srcs`"))]
          (when generated? (.delete build-file))
          {:files 0 :wrote 0 :deleted (if generated? 1 0)})))))
...
(defn gen-source-paths-
  ...
  (let [results (->> paths
    ...
    (reduce (fn [acc m] (merge-with + acc m))
            {:files 0 :wrote 0 :deleted 0 :dirs (count results)}
            results)))
...
(defn -main [& args]
  ...
  (let [cmd (first args)
    ...
    (let [result (f opts)]
      (when (map? result)
        (println (format "gen_srcs: checked %d files across %d dirs, wrote %d BUILD files, deleted %d BUILD files"
                         (:files result) (:dirs result) (:wrote result) (:deleted result))))
      (shutdown-agents))))
