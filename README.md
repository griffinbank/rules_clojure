...
Since `clojure_tools_deps` only downloads jars and doesn't automatically included them in any clojure target, there is no harm in including all `:aliases` in your project
...
`gen_srcs` also creates a few extra targets in every directory on the deps.edn search path. It will produce `clojure_library` targets named `__clj_lib`  and `__cljs_lib` containing all source files in the directory (non-AOT'd), and all subpackages, and `filegroup` rules named `__clj_files` and `__cljs_files`. ```//src:__clj_files``` includes all src files under `src`. This target is useful for static analysis tools.
...
Currently the clojurescript compiler hardcodes the path `./node_modules`, so bazel-managed node modules isn't supported yet.

gen-build does not yet correctly generate fine-grained dependencies for CLJS, so you'll need to use either the coarse ``//src:__cljs_lib` and `@deps//:__all`, or write BUILD files by hand for now.
