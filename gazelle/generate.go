...
// fallback, rollup composition) live in the bb server's ns-rules
// (src/rules_clojure/gazelle_server.bb). This file is the wire-format
// translator + the Gazelle integration layer; it doesn't decide what a
// clojure_library should look like.
package gazelle
...
// of rel ("" if none match). Used for source-path / ignore-path lookup.
...
// buildRule translates a {kind, attrs} spec from the bb server's ns-rules
// into a Gazelle *rule.Rule. The bb side is authoritative on what rules
// to emit and which attrs to set; this is purely a wire-format translator.
// Returns an error rather than panicking on malformed specs so the caller
// can fail the whole run loudly (vs. silently skipping rules).
...
func applyAttr(r *rule.Rule, key string, v interface{}) error {
	...
	case float64:
		if val != math.Trunc(val) {
			return fmt.Errorf("non-integer float %v for attr %q (Bazel int attrs must round-trip exactly)", val, key)
		}
	...
	case map[string]interface{}:
		...
		// (e.g. clojure_test :env). All-string is the only shape ns-rules in
		// gazelle_server.bb emits — fail loudly on anything else so a future
		// change to a dict with non-string values surfaces here.
...
func (l *clojureLang) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	if pathUnder(args.Rel, session.initResp.IgnorePaths) != "" {
		return language.GenerateResult{}
	}
	if pathUnder(args.Rel, session.initResp.SourcePaths) == "" {
		return language.GenerateResult{}
	}
	...
	for _, sub := range args.Subdirs {
		...
		if pathUnder(subRel, session.initResp.SourcePaths) == "" {
			continue
		}
